...
Compiler setups may replace source reading before parsing with a read metaprogram.
The setup module declares `(reader-provider "namespace" function)`; the provider is
a normal `[(context Code)] -> Code` compile-time function. Its context is
`(read-context PATH SOURCE KIND)`. It may return arbitrary compiler syntax or delegate
to the configurable built-in s-expression reader with
`(primitive/code-read SOURCE `(reader-config :unquote #\, :splice #\@))`.
Only the small provider namespace is compiled during this bootstrap phase. Namespace
imports continue to use Coil's default reader; textual Scheme `load` inherits Scheme
reader punctuation.
...
Conventional `\n`, `\t`, `\r`, `\b`, and `\f` escapes denote newline, tab,
return, backspace, and form feed.
...
`code-list?` means a **proper** list ending in nil, so it is the guard for
`code-count` and indexed access. `code-pair?` recognizes any non-empty cons,
including an improper/dotted pair; traverse arbitrary pair structure with
`code-car`/`code-cdr`. This makes `derive` (`derive.coil`:
...
program:
    ...
    (MARKER "my.module.expand-language-form" FORM)
...
transform explicitly requests an invocation with `(MARKER ENTRY ARG...)`. `MARKER`
is an opaque symbol selected by the transform, normally a fresh
`(primitive/gensym)`, and must be the same symbol declared by `stage`. `ENTRY` is a
symbol or string naming the phase entry (a fully-qualified string is the unambiguous
form). The marked list is replaced by the entry's returned `Code`, and the result starts the **next
syntax-transform round**. It does not jump ahead to ordinary Coil expansion.

The explicit marker identity is important. The compiler executes only lists headed by
a marker declared in a `stage`; it does not reserve a global staging form or treat every
list whose head matches an entry as a call. Only the language metaprogram knows whether
a list is code, quoted data, shadowed syntax, or not an s-expression language at all.
Nor does the
...
Marker identities remain available in later rounds and are not scoped to the module
that declared them. A transform can therefore deliberately declare phase code in one
module and route a request from another module, while unrelated source cannot
accidentally spell a fresh marker. `(stage MARKER)` is valid when a later round needs to
introduce no new phase code but still contains requests using that marker.
...
The isolated phase program itself is cumulative, not only its entry table. Forms from a
later `stage` resolve and typecheck together with earlier stage forms, so a new entry can
call an earlier helper or entry as ordinary compiled Coil. Only newly introduced entries
are installed into the phase invocation table after each rebuild.

A transform should emit a stage declaration only in a round containing requests for its
marker. Once those requests expand, the next transform round must stop emitting that
declaration. As with every syntax transform, failure to converge within 16 rounds is
diagnosed.
