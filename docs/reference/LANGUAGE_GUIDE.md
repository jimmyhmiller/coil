...
`coil.result`, and `coil.assert.hermetic`. Imports of other bundled namespaces
are rejected while loading the complete reachable graph, including imports made
by a core provider. Authored `extern` declarations, `llvm-ir`, native dependencies,
and `[link]` inputs are also rejected. Compiler-owned bundled modules remain able
to use sealed lowering intrinsics while implementing the admitted surface.
