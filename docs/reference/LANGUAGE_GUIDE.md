...
Every invocation also runs against a **budget** (default 64 MiB). A runaway
expansion dies in milliseconds with an error naming the metaprogram, the
bytes, and the fix — never a mystery multi-gigabyte compile. A metaprogram
that legitimately needs more declares it, in its own module, as a reviewable
line of source:

    (meta-budget read-brainfuck 1024)   ; this reader scales with its input
