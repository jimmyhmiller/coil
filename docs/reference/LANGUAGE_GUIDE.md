...
- the link inputs, native dependencies, and `[cc]` sources contributed by dependency
  packages, since the code it imports needs them;
...
`def` needs them declared with `:params`. Each entry is a name, or
`(:positional name)` for a slot a named call cannot address. The call is ordered
exactly as a function's would be, with arguments still evaluated in source order:

    (def hot :params [x]
      (var-static (fnptr c [i64] i64) (primitive/fnptr-of old-code)))
    (hot :x 10)

The REPL publishes every `defn` this way, which is why `(double :x 3)` works there.
The names are part of a REPL function's published signature: a redefinition may
change the body but not the parameter names or types.
...
bindings with static storage. `(def NAME :params [p …] …)` additionally names the
parameters of a callable value for named calls (see `Var`). The initializer is evaluated as a link-time constant;
