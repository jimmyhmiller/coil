...
RUN_DIR=$(mktemp -d /tmp/coil-bootstrap-x64.XXXXXX) || exit 1
trap 'stage_lib_cleanup; rm -rf "$RUN_DIR"' EXIT
S1="$RUN_DIR/coil-x64-s1"; S2="$RUN_DIR/coil-x64-s2"; S3="$RUN_DIR/coil-x64-s3"
...
"$STAGE0" build "$SRC" -o "$S1" "${LF[@]}" || { echo "stage1 FAILED"; exit 1; }
echo "=== stage2: stage1 rebuilds it with --backend x64 ==="
"$S1" build "$SRC" -o "$S2" --backend x64 "${LF[@]}" || { echo "stage2 FAILED"; exit 1; }
echo "=== stage3: stage2 rebuilds it with --backend x64 ==="
"$S2" build "$SRC" -o "$S3" --backend x64 "${LF[@]}" || { echo "stage3 FAILED"; exit 1; }
...
./scripts/compiler/oracle/x64/gate-cabi.sh "$S2" >/dev/null 2>&1 \
...
if command -v llvm-mc >/dev/null 2>&1; then
  ./scripts/compiler/oracle/x64/gate-encode.sh "$S2" >/dev/null 2>&1 \
