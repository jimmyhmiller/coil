...
| `fuzz_demo.coil` | **coverage-guided fuzzing**: a 4-byte magic value behind nested branches. `coil test` never finds it (~10^8 cases by construction); guided search finds it on 5 seeds of 5. Run with `scripts/tests/prop-fuzz.sh tests/prop/demos/fuzz_demo.coil` | `s = "FUZZ"`, in 6k–85k cases |
