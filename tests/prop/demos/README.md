...
| `fuzz_demo.coil` | **coverage-guided fuzzing**: a 4-byte magic value behind nested branches. `coil test` never finds it (~10^8 cases by construction); guided search finds it on 5 seeds of 5. Run with `coil fuzz tests/prop/demos/fuzz_demo.coil` | `s = "FUZZ"`, in ~2k–12k cases (edge coverage alone: 6k–85k) |
| `json_fuzz_demo.coil` | the same machinery pointed at a REAL parser (`coil.json`), with the two laws worth asserting about any parser: it never crashes, and an accepted input is self-consistent | passes — and reaches 173 of 440 edges under guidance against 96 blind |
| `memory_bug_demo.coil` | a real off-by-one read, found by generated inputs and minimized. **Needs `coil test --sanitize=address --alloc-tracked …`** — the arena hides small overflows from the sanitizer, and without the sanitizer the read is harmless | `heap-buffer-overflow … 0 bytes after 1-byte region`, `s = "a"` |
