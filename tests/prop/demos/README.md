...
| `hang_demo.coil` | a property that **never finishes** is minimized the same way, via the watchdog | `a = 10`…`12`, "the property NEVER FINISHED on this input" — run with `--timeout 3 --candidate-timeout 1`, and expect it to take a minute: each candidate costs its whole timeout |
...
| `fuzz_demo.coil` | **coverage-guided fuzzing**: a 4-byte magic value behind nested branches. `coil test` never finds it (~10^8 cases by construction); a campaign finds it in seconds. Run with `coil fuzz tests/prop/demos/fuzz_demo.coil` | `s = "FUZZ"`, after a few thousand executions |
| `json_fuzz_demo.coil` | the same machinery pointed at a REAL parser (`coil.json`), with the two laws worth asserting about any parser: it never crashes, and an accepted input is self-consistent. `tests/fuzz/json_fuzz.coil` is the full target | passes; `.coil/fuzz/<property>/coverage.txt` shows what the campaign reached |
| `memory_bug_demo.coil` | a real off-by-one read, found by generated inputs and minimized. **Needs `coil test --sanitize=address --alloc-tracked …`** (or `coil fuzz --sanitize=address`, which does both) — the arena hides small overflows from the sanitizer, and without the sanitizer the read is harmless | `heap-buffer-overflow … 0 bytes after 1-byte region`, `s = "a"` |
